...
export default function Home() {
  ...
  return (
    <div className="overflow-hidden">
      <section className="relative isolate min-h-[620px] overflow-hidden bg-brand-ink text-white"><motion.img initial={reduceMotion ? false : { scale: 1.06 }} animate={reduceMotion ? undefined : { scale: 1 }} transition={{ duration: 1.2, ease: 'easeOut' }} src={img1} alt="Femmes participant à une activité communautaire de l’AFD" className="absolute inset-0 -z-20 h-full w-full object-cover" /><div className="absolute inset-0 -z-10 bg-gradient-to-r from-brand-ink via-brand-deep/90 to-brand-deep/20" /><Container className="flex min-h-[620px] items-center py-24"><motion.div initial={reduceMotion ? false : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: 'easeOut' }} className="max-w-3xl"><Badge className="bg-brand-gold text-brand-ink">Engagées depuis {settings.founded_year}</Badge><h1 className="mt-7 font-display text-5xl font-bold leading-[1.02] tracking-[-0.045em] sm:text-6xl lg:text-7xl">Des femmes actrices du développement de leurs communautés.</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-white/80 sm:text-xl">L’Alliance des Femmes pour le Développement agit avec les communautés pour la dignité, la résilience et des opportunités durables en RDC.</p><div className="mt-10 flex flex-col gap-3 sm:flex-row"><Link to="/programmes"><Button variant="secondary">Découvrir nos actions <ArrowRight className="h-4 w-4" /></Button></Link><Link to="/don"><Button className="border-white/40 text-white hover:bg-white/10" variant="outline">Soutenir l’AFD</Button></Link></div><p className="mt-12 text-sm font-semibold text-white/75">Présente dans {settings.provinces_count ?? 'plusieurs'} provinces de la RDC</p></motion.div></Container></section>

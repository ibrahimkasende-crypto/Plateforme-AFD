...
| test:rls (`AFD_REQUIRE_RLS=1`) | **OK** 3/3 (via `.env.local` → projet `ancien-projet-supabase`) |
...
**Note :** les tests RLS locaux ne certifient pas encore AFD (`mxxuxnoqnwjygawvvhcb`) tant que `.env.local` pointe ailleurs.

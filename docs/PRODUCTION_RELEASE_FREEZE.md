...
Figer le périmètre fonctionnel tant que Hostinger, domaine, variables d’environnement, projet Supabase `mxxuxnoqnwjygawvvhcb` et backup ne sont pas validés.
...
| Seeds présentation / HR démo | Ne pas exécuter contre AFD prod |
...
- [ ] Env prod alignée sur `mxxuxnoqnwjygawvvhcb`  
- [ ] `/api/health` live = `ok`  
